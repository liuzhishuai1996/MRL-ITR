#h(t)=exp(p*t+q)*exp(A*(-c*t+d)*(x1-x2))，
#denote A*(x1-x2) as l，requiring p-c*l>0
#S(t)=exp(1/(c*l-p)(exp((p-c*l)*t+(q+l*d))-exp(q+l*d)))=U
...
fileName <- paste('time-varying regime', 'AIPW', 'proportional', N, PS, t, CR, sep = '-')
